...
The full event-disjoint training, validation, and test arrays is available
from [Zenodo record 22150295](https://zenodo.org/records/22150295). The record is
currently unpublished pending double-blind peer review.

No download is needed for the tutorial: `ma_pfn_tutorial.npz` contains a
checksum-verified subset of the real event pairs and exact EMD targets. The
notebook extracts it automatically when full arrays are not present.

For the full workflow, place the six Zenodo arrays in `data/`:
